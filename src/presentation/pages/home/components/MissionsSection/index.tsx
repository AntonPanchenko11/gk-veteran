...
const missionCards: MissionCard[] = [
  ...
  {
    title: 'Демонстрации того, что труд ветеранов необходим и ценен для общества сегодня и в будущем',
    image: missionImage7,
    gridClass: 'md:col-span-2',
  },
]
...
export function MissionsSection() {
  return (
    <section id="missions" className="bg-[#202020]">
      <div className="mx-auto max-w-6xl px-6 py-10 lg:px-12">
        ...
        <div className="mt-5 grid gap-6 md:auto-rows-[minmax(220px,_auto)] md:grid-cols-6">
          {missionCards.map(({ title, image, gridClass }) => (
            <article
              key={title}
              className={`relative flex min-h-[220px] w-full flex-col justify-end overflow-hidden rounded-[28px] bg-[#4F5A4A] p-8 text-left text-white shadow-[0_40px_80px_-45px_rgba(0,0,0,0.9)] md:min-h-0 ${gridClass}`}
            >
              <img
                src={image}
                alt=""
                className="absolute left-1/2 top-1/2 h-[200px] w-[300px] -translate-x-1/2 -translate-y-1/2 object-contain"
                loading="lazy"
              />
              <div
                ...
                aria-hidden="true"
                style={{
                  background: 'linear-gradient(180deg, rgba(79, 90, 74, 0) 45%, rgba(79, 90, 74, 0.92) 100%)',
                }}
              ></div>
